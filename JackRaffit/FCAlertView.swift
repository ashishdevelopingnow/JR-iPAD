...
//     numberOfButtons == 1 && !hideDoneButton &&
  ...
  override public func draw(_ rect: CGRect) {
    ...
    if(!hideAllButtons && !hideDoneButton && numberOfButtons == 0) {
      ...
      doneButton.addTarget(self, action: #selector(FCAlertView.donePressed(sender:)), for: .touchUpInside)
     ...
     otherButton.addTarget(self, action:  #selector(FCAlertView.handleButton(sender:)), for: .touchUpInside)
      ...
      firstButton.setTitle(buttonTitles![0], for: .normal)
        firstButton.addTarget(self, action: #selector(FCAlertView.handleButton(sender:)), for: .touchUpInside)
      ...
      secondButton.addTarget(self, action: #selector(FCAlertView.handleButton(sender:)), for: .touchUpInside)
    ...
    let alertViewVector = UIButton(type: .custom)
    alertViewVector.frame = CGRect(x:alertViewContents!.frame.size.width/2 - 15.0,
                                   y:-15.0,
                                   width:30.0,
                                   height:30.0)
    alertViewVector.setBackgroundImage(vectorImage, for: .normal)
    alertViewVector.isUserInteractionEnabled = false
    alertViewVector.imageView?.contentMode = UIViewContentMode.scaleAspectFit
//    alertViewVector.tintColor = UIColor.lightGray
  ...
  private func makeAlertTypeSuccess(){
    if let path = Bundle(for: FCAlertView.self).path(forResource: "checkmark-round", ofType: "png") {
      setTheme(iconPath: path, tintColor: .themeColor)
    }
  }
